...
void SetupFog(int renderDistance) {
  glFogColor(31, 31, 31, 31);
  glFogShift(2);
  glFogOffset(27800);
...
void startingDraw(bool cullback) {
  ...
  // SetupFog(RENDER_DISTANCE);

  glMaterialf(GL_AMBIENT, RGB15(25, 25, 25));
  glBegin(GL_QUADS);
}

void drawCubeTop(vec2_t t, u8 angle) {
  ...
  default:
    ...
    glVertex3v16(inttov16(1) + ARR, inttov16(1) + ARR, 0);

    glTexCoord2t16(inttot16(u1), inttot16(v0));
    glVertex3v16(0, inttov16(1) + ARR, 0);

    glTexCoord2t16(inttot16(u0), inttot16(v0));
    glVertex3v16(0, inttov16(1) + ARR, inttov16(1) + ARR);

    glTexCoord2t16(inttot16(u0), inttot16(v1));
    glVertex3v16(inttov16(1) + ARR, inttov16(1) + ARR, inttov16(1) + ARR);
    break;
...
static void drawBar(v16 minX, v16 minY, v16 minZ, v16 maxX, v16 maxY,
                    v16 maxZ) {
  glBegin(GL_QUADS);
...
void drawBlockOutline(int x, int y, int z) {
  ...
  // Bas avant
  drawBar(MIN, MIN, MIN, MAX, OUTLINE_SIZE, OUTLINE_SIZE);

  // Bas arrière
  drawBar(MIN, MIN, MAX - OUTLINE_SIZE, MAX, OUTLINE_SIZE, MAX);

  // Haut avant
  drawBar(MIN, MAX - OUTLINE_SIZE, MIN, MAX, MAX, OUTLINE_SIZE);

  // Haut arrière
  drawBar(MIN, MAX - OUTLINE_SIZE, MAX - OUTLINE_SIZE, MAX, MAX, MAX);

  // Avant gauche
  drawBar(MIN, MIN, MIN, OUTLINE_SIZE, MAX, OUTLINE_SIZE);

  // Avant droite
  drawBar(MAX - OUTLINE_SIZE, MIN, MIN, MAX, MAX, OUTLINE_SIZE);

  // Arrière gauche
  drawBar(MIN, MIN, MAX - OUTLINE_SIZE, OUTLINE_SIZE, MAX, MAX);

  // Arrière droite
  drawBar(MAX - OUTLINE_SIZE, MIN, MAX - OUTLINE_SIZE, MAX, MAX, MAX);

  // Bas gauche
  drawBar(MIN, MIN, MIN, OUTLINE_SIZE, OUTLINE_SIZE, MAX);

  // Bas droite
  drawBar(MAX - OUTLINE_SIZE, MIN, MIN, MAX, OUTLINE_SIZE, MAX);

  // Haut gauche
  drawBar(MIN, MAX - OUTLINE_SIZE, MIN, OUTLINE_SIZE, MAX, MAX);

  // Haut droite
  drawBar(MAX - OUTLINE_SIZE, MAX - OUTLINE_SIZE, MIN, MAX, MAX, MAX);
